...
create table if not exists public.support_tickets (
  ...
  status text not null default 'new'
    check (status in ('new', 'triaging', 'waiting_on_user', 'in_progress', 'escalated', 'resolved', 'closed')),
  assigned_to text,
...
on public.support_tickets (category_id, status);
...
grant insert, update on public.support_tickets to service_role;
...
drop policy if exists "Users can select own support tickets" on public.support_tickets;
...
drop view if exists public.support_ticket_overview;
...
select
  ...
  ticket.id,
  ticket.user_id,
  coalesce(profile.username, split_part(auth_user.email, '@', 1), split_part(ticket.reply_email, '@', 1)) as username,
  coalesce(auth_user.email, ticket.account_email) as account_email,
  ticket.reply_email,
  ticket.category_id,
...
left join public.user_profiles profile on profile.id = ticket.user_id
