...
import {
  ...
  getRoguelikeDifficultyTokenMultiplier,
...
type TradeReplacementMode = "similar" | "fit";
...
interface RoguelikeRun {
  ...
  pendingTradeState?: {
    ...
    outgoingPlayerTier?: PlayerTier;
    replacementMode?: TradeReplacementMode | null;
...
const getOverallBoostAmountForTrainingNode = (node: RoguelikeNode | null | undefined) => {
  if (!node) return 0;
  if (node.id === LOCKER_ROOM_SPECIAL_STUFF_NODE.id) return SPECIAL_STUFF_BOOST_AMOUNT;
  if (node.id === STORE_TRAINING_NODE.id || node.id === LOCKER_ROOM_TRAINING_NODE.id) return 1;
  if (node.type === "training" && !node.id.startsWith("locker-room-practice-")) return 1;
  return 0;
};
...
const getDifficultyTokenRewardLabel = (difficulty: RoguelikeDifficulty) => {
...
const injectRogueStarterSlotsIntoReveal = (starterRevealPlayers: Player[], starterSlotPlayers: Player[]) => {
  if (starterSlotPlayers.length === 0) return starterRevealPlayers;

  const uniqueSlotPlayers = starterSlotPlayers.filter(
    (player, index, players) => players.findIndex((candidate) => candidate.id === player.id) === index,
  ).slice(0, 3);
  const naturalPlayers = starterRevealPlayers.filter(
    (player) => !uniqueSlotPlayers.some((slotPlayer) => slotPlayer.id === player.id),
  );

  return [...uniqueSlotPlayers, ...naturalPlayers].slice(0, 3);
};
...
const getOpeningLockerCashForTier = (tier: number) => {
  if (tier >= 3) return 10;
  if (tier === 2) return 6;
  if (tier === 1) return 3;
  return 0;
...
const getLegacyRoguelikeNodesForSettings = (
  ...
  getRoguelikeNodesForSettings(settings, { enableCoachRecruitment })
    .filter((node) => ladderVersion >= 5 || node.type !== "chance")
...
const normalizeStoredRun = (parsed: Partial<RoguelikeRun>, activeRogueStarId: string | null): RoguelikeRun | null => {
  ...
  return {
    ...
    starterKeptIds: parsed.starterKeptIds ?? [],
...
const buildRevealedStarterRosterState = (run: RoguelikeRun, keptStarterIds: string[]) => {
  const revealedPlayers = run.starterRevealPlayers.filter((player) => keptStarterIds.includes(player.id));
  return {
...
const getRunOwnedPlayers = (run: Pick<RoguelikeRun, "roster" | "lineup">) => {
...
const getSpecialStuffTrainingIds = (playerId: string | null | undefined, boostAmount: number) =>
  playerId && boostAmount > 0 ? Array.from({ length: boostAmount }, () => playerId) : [];

const getUncappedRunDisplayOverall = (
  player: Player,
  ownedPlayerIds: string[] = [],
  trainedPlayerIds: string[] = [],
  coachTeamKey: string | null = null,
) => player.overall + getRoguelikeDisplayOverallBonus(player, ownedPlayerIds, trainedPlayerIds, coachTeamKey);

const canApplyRunOverallBoost = (
  player: Player,
  boostAmount: number,
  ownedPlayerIds: string[] = [],
  trainedPlayerIds: string[] = [],
  coachTeamKey: string | null = null,
) => getUncappedRunDisplayOverall(player, ownedPlayerIds, trainedPlayerIds, coachTeamKey) + boostAmount <= ROGUELIKE_MAX_OVERALL;

const getAvailableRunOverallBoostAmount = (
  player: Player,
  boostAmount: number,
  ownedPlayerIds: string[] = [],
  trainedPlayerIds: string[] = [],
  ...
  Math.max(
    ...
    Math.min(
      boostAmount,
      ROGUELIKE_MAX_OVERALL - getUncappedRunDisplayOverall(player, ownedPlayerIds, trainedPlayerIds, coachTeamKey),
    ),
  );

const getEffectiveBossTrainedPlayerIds = (
  run: Pick<
    RoguelikeRun,
    "trainedPlayerIds" | "activeSpecialStuffPlayerId" | "roster" | "lineup" | "hiredCoachId"
  >,
) => {
  const trainedPlayerIds = run.trainedPlayerIds ?? [];
  const specialStuffPlayerId = run.activeSpecialStuffPlayerId;
  if (!specialStuffPlayerId) return trainedPlayerIds;

  const ownedPlayers = getRunOwnedPlayers(run);
  const specialStuffPlayer = ownedPlayers.find((player) => player.id === specialStuffPlayerId);
  if (!specialStuffPlayer) return trainedPlayerIds;

  const specialStuffBoostAmount = getSpecialStuffBoostAmountForPlayer(
    specialStuffPlayer,
    ownedPlayers.map((player) => player.id),
    trainedPlayerIds,
  ...
  return [
    ...trainedPlayerIds,
    ...getSpecialStuffTrainingIds(specialStuffPlayerId, specialStuffBoostAmount),
  ];
};

const getSpecialStuffBoostAmountForPlayer = (
  player: Player,
  ownedPlayerIds: string[] = [],
  trainedPlayerIds: string[] = [],
  coachTeamKey: string | null = null,
) =>
  getAvailableRunOverallBoostAmount(
    player,
    SPECIAL_STUFF_BOOST_AMOUNT,
    ownedPlayerIds,
    trainedPlayerIds,
    coachTeamKey,
  );
...
const getTradePreviewOverall = (
  ...
  return capRoguelikeOverall(
    player.overall + getRoguelikeDisplayOverallBonus(player, previewOwnedPlayerIds, [], coachTeamKey),
  );
};

const TRADE_FOR_FIT_START_NODE_ID = "year-2-finals";
const PLAYER_TIER_ORDER: PlayerTier[] = ["Emerald", "Sapphire", "Ruby", "Amethyst", "Galaxy"];

const getPlayerTierIndex = (tier: PlayerTier) => PLAYER_TIER_ORDER.indexOf(tier);

const getLowerPlayerTiers = (tier: PlayerTier) => {
  const tierIndex = getPlayerTierIndex(tier);
  return tierIndex > 0 ? PLAYER_TIER_ORDER.slice(0, tierIndex) : [];
};
...
const getRunDisplayPlayer = (
  ...
  return {
    ...player,
    overall: capRoguelikeOverall(player.overall + displayOverallBonus),
...
const CHANCE_WHEEL_TEAMS = nbaTeams.map((team) => team.name);
const CHANCE_WHEEL_SLICE_COLORS = [
  "#ef4444",
  "#2563eb",
  "#22c55e",
  "#f59e0b",
...
const chanceWheelBackground = `conic-gradient(${CHANCE_WHEEL_TEAMS.map((_, index) => {
  ...
  const end = ((index + 1) / CHANCE_WHEEL_TEAMS.length) * 100;
  return `${CHANCE_WHEEL_SLICE_COLORS[index % CHANCE_WHEEL_SLICE_COLORS.length]} ${start}% ${end}%`;
}).join(", ")})`;

const getChanceWheelTeamLabel = (teamName: string) => {
  const segments = teamName.split(" ");
  return segments[segments.length - 1] ?? teamName;
};

const getChanceWheelTierLabel = (node: RoguelikeNode | null) => {
  const tiers = node ? getNodeChoiceTiers(node) : null;
  if (!tiers?.length) return "eligible";
  return tiers.length === 1 ? tiers[0] : tiers.join("/");
};

const getChanceWheelTeamName = (run: RoguelikeRun, node: RoguelikeNode) => {
  if (run.chanceWheelTeamName) return run.chanceWheelTeamName;

  const rng = mulberry32(nextChoiceSeed(run.seed, 720 + run.floorIndex * 41 + node.id.length));
  return CHANCE_WHEEL_TEAMS[Math.floor(rng() * CHANCE_WHEEL_TEAMS.length)] ?? CHANCE_WHEEL_TEAMS[0];
};

const getChanceWheelPlayerPool = (
  ...
  return [...pool, ...fallbackPool].filter((player) => {
    if (seenIds.has(player.id)) return false;
    if (getPlayerTeamKey(player) !== teamName) return false;
...
const getTradeForFitConnectionLabels = (
  candidate: Player,
  nextRoster: Player[],
  coachTeamKey: string | null = null,
) => {
  const labels: string[] = [];
  const candidateTeamKey = getPlayerTeamKey(candidate);
  const rosterPlayerIds = nextRoster.map((player) => player.id);
  const proposedPlayerIds = rosterPlayerIds.includes(candidate.id)
    ? rosterPlayerIds
    : [...rosterPlayerIds, candidate.id];

  if (coachTeamKey && candidateTeamKey === coachTeamKey) {
    labels.push("Coach Link");
  }

  if (
    candidateTeamKey &&
    nextRoster.some((player) => getPlayerTeamKey(player) === candidateTeamKey) &&
    getSameTeamChemistryBonusForPlayer(candidate, proposedPlayerIds) > 0
  ) {
    labels.push(`Same Team: ${candidateTeamKey}`);
  }

  getPlayerBadgeStates(candidate.id, rosterPlayerIds).forEach((badgeState) => {
    if (!badgeState.previewActive) return;
    if (badgeState.active) return;

    if (badgeState.definition.type === "dynamic-duo") {
      labels.push("Dynamic Duo");
      return;
    }

    if (badgeState.definition.type === "team-chemistry") {
      labels.push("Team Chemistry");
      return;
    }

    labels.push(badgeState.definition.title);
  });

  return Array.from(new Set(labels));
};

const getTradeForFitPool = (
  pool: Player[],
  fallbackPool: Player[],
  outgoingPlayerTier: PlayerTier,
  nextRoster: Player[],
  ...
  const lowerTiers = new Set(getLowerPlayerTiers(outgoingPlayerTier));
  if (lowerTiers.size === 0) return [];

  const seenIds = new Set<string>();
  const seenIdentities = new Set<string>();

  return [...pool, ...fallbackPool].filter((candidate) => {
    if (candidate.id === outgoingPlayerId) return false;
    if (!lowerTiers.has(getPlayerTier(candidate))) return false;
    if (isPlayerIdentityOnRoster(candidate, nextRoster)) return false;
    if (getTradeForFitConnectionLabels(candidate, nextRoster, coachTeamKey).length === 0) return false;
...
const drawTradeForFitChoices = (
  tradeForFitPool: Player[],
  nextRoster: Player[],
  count: number,
  seed: number,
  coachTeamKey: string | null = null,
  ...
  const shuffledPool = tradeForFitPool.filter((candidate) => !blockedIds.has(candidate.id));
  ...
  return shuffledPool
    .sort((left, right) => {
      const leftConnectionCount = getTradeForFitConnectionLabels(left, nextRoster, coachTeamKey).length;
      const rightConnectionCount = getTradeForFitConnectionLabels(right, nextRoster, coachTeamKey).length;
      if (leftConnectionCount !== rightConnectionCount) return rightConnectionCount - leftConnectionCount;

      const leftTierIndex = getPlayerTierIndex(getPlayerTier(left));
      const rightTierIndex = getPlayerTierIndex(getPlayerTier(right));
      if (leftTierIndex !== rightTierIndex) return rightTierIndex - leftTierIndex;
...
const canNodeOfferTradeForFit = (
  node: RoguelikeNode | null,
  nodes: RoguelikeNode[],
  pendingChoiceSelection?: "training" | "trade" | null,
) => {
  if (!node) return false;
  if (node.type !== "trade" && !(node.type === "choice" && pendingChoiceSelection === "trade")) return false;

  const yearTwoFinalsIndex = nodes.findIndex((entry) => entry.id === TRADE_FOR_FIT_START_NODE_ID);
  const nodeIndex = nodes.findIndex((entry) => entry.id === node.id);
  return yearTwoFinalsIndex >= 0 && nodeIndex > yearTwoFinalsIndex;
};
...
const getActDescription = (act: number) => {
  ...
  return "Only the strongest builds survive the finals stretch. Every floor now needs to feel worthy of a championship run.";
};
...
const getRoguelikeNodeTypeTheme = (type: RoguelikeNode["type"]) => {
  ...
  > = {
    ...
    chance: {
      label: "Team Wheel",
      Icon: Sparkles,
      chip: "border-yellow-200/18 bg-yellow-300/10 text-yellow-100",
      iconWrap: "border-yellow-200/22 bg-yellow-300/12",
      iconColor: "text-yellow-100",
      accentLine: "from-yellow-300/90 via-emerald-300/30 to-transparent",
      summary:
        "border-yellow-200/22 bg-[linear-gradient(135deg,rgba(250,204,21,0.16),rgba(15,23,42,0.86),rgba(16,185,129,0.14))] text-yellow-50",
    },
    evolution: {
      label: "Evolution",
      Icon: Sparkles,
...
const drawRunChoices = (
  ...
  if (choices.length < count && relaxTierFallback) {
    choices = drawRoguelikeChoices(
      pool,
      roster,
      count,
...
const getRewardDraftPool = (
  ...
  if (node.type === "chance") {
    return getChanceWheelPlayerPool(run.chanceWheelTeamName, pool, fallbackPool);
  }
...
const getRewardDraftDescription = (node: RoguelikeNode, choiceCount = node.rewardChoices) => {
  ...
  if (node.type === "chance") {
    return `Spin the 30-team wheel, then choose 1 of ${choiceCount} ${tierLabel} players from the team it lands on.`;
  }
  ...
  return "Take the next run reward tied to this floor.";
};
...
const getNodeCompletionRewardCopy = (node: RoguelikeNode) => {
  ...
  if (node.type === "chance") {
    return {
      title: "Team Wheel",
      description: `Spin the NBA team wheel, then choose 1 ${getChanceWheelTierLabel(node)} player from the team it lands on.`,
    };
  }
...
const FaceoffStarterCard = ({
  ...
  return (
    <div
      className={clsx(
        "relative overflow-hidden rounded-[20px] border px-4 py-3 transition",
        highlighted
          ? "border-emerald-200/40 bg-[linear-gradient(135deg,rgba(16,185,129,0.18),rgba(255,255,255,0.06),rgba(16,185,129,0.08))] shadow-[0_0_0_1px_rgba(110,231,183,0.16),0_18px_46px_rgba(16,185,129,0.16)]"
          : "border-white/10 bg-white/5",
      )}
    >
      {highlighted ? (
        <div className="pointer-events-none absolute inset-y-3 left-0 w-[3px] rounded-full bg-emerald-300 shadow-[0_0_18px_rgba(52,211,153,0.5)]" />
      ) : null}
      ...
      {player && boxScoreLine ? (
        <div className={clsx(
          "mt-3 rounded-[16px] border border-white/10 bg-black/18 px-3 py-2",
          align === "right" ? "text-left" : "text-right",
        )}>
          <div className="text-[9px] uppercase tracking-[0.18em] text-slate-500">Box Score</div>
          <div className="mt-1 text-sm font-semibold leading-5 text-white">{boxScoreLine}</div>
        </div>
      ) : null}
      {highlighted ? (
        <div
          className={clsx(
            "mt-3 inline-flex rounded-full border border-emerald-200/24 bg-emerald-300/12 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-100",
...
const FaceoffMatchupRow = ({
  ...
  const formatBreakdownScore = (value: number) => `${Math.round(value * 10) / 10}`;
  const bossSupport =
  ...
  const projectedUserWins = matchup.userRating >= matchup.opponentRating;
  const projectedWinnerName = projectedUserWins
    ? matchup.userPlayer?.name ?? "Your player"
    : matchup.opponentPlayer?.name ?? "Boss player";
  const projectedEdgeLabel =
    Math.abs(matchup.ratingDelta) >= 12
      ? "Heavy Advantage"
      : Math.abs(matchup.ratingDelta) >= 5
        ? "Clear Advantage"
        : Math.abs(matchup.ratingDelta) >= 1.5
          ? "Slight Advantage"
          : "Toss-Up";
  const projectionReasonRows = [
    {
      label: "Talent",
      userValue: matchup.userBreakdown.baseScore,
      opponentValue: matchup.opponentBreakdown.baseScore,
      signed: false,
    },
    {
      label: "Team Help",
      userValue: userSupport,
      opponentValue: bossSupport,
      signed: true,
    },
    {
      label: "Badge Edge",
      userValue: matchup.userBreakdown.badgeMatchupBonus,
      opponentValue: matchup.opponentBreakdown.badgeMatchupBonus,
      signed: true,
    },
    {
      label: "Matchup Fit",
      userValue: matchup.userBreakdown.headToHeadBonus,
      opponentValue: matchup.opponentBreakdown.headToHeadBonus,
      signed: true,
    },
  ];
  const userStat = matchup.userPlayer
    ? playerStats.find((stat) => stat.team === "user" && stat.playerId === matchup.userPlayer?.id)
    : null;
  const opponentStat = matchup.opponentPlayer
    ? playerStats.find((stat) => stat.team === "opponent" && stat.playerId === matchup.opponentPlayer?.id)
    : null;
  const getDuelScore = (stat: RoguelikeSimulationPlayerStat | null | undefined) => {
    if (!stat) return 0;
    const efficiencyLift =
      stat.fieldGoalsAttempted > 0
        ? (stat.fieldGoalsMade / stat.fieldGoalsAttempted - 0.45) * 12
        : 0;
    return Math.round(
      stat.points +
        stat.assists * 1.7 +
        stat.rebounds * 1.2 +
        (stat.steals + stat.blocks) * 2.2 -
        stat.turnovers * 1.6 +
        efficiencyLift,
    );
  };
  const userDuelScore = getDuelScore(userStat);
  const opponentDuelScore = getDuelScore(opponentStat);
  const hasActualDuel = Boolean(userStat && opponentStat);
  const actualUserWins = userDuelScore >= opponentDuelScore;
  const highlightedUserWins = hasActualDuel ? actualUserWins : projectedUserWins;
  const winnerHighlightLabel = hasActualDuel ? "Won Matchup" : "Projected Edge";
  const actualWinnerName = actualUserWins
    ? matchup.userPlayer?.name ?? "Your player"
    : matchup.opponentPlayer?.name ?? "Boss player";
  const projectionMatchedActual = hasActualDuel && projectedUserWins === actualUserWins;
  const matchupHeadline = hasActualDuel
    ? `${actualWinnerName} Won Matchup`
    : `${projectedWinnerName} Favored`;
  const statLine = (stat: RoguelikeSimulationPlayerStat | null | undefined) =>
    stat ? `${stat.points} PTS / ${stat.assists} AST / ${stat.rebounds} REB` : "No box score";
    ...
    <div className="rounded-[26px] border border-white/10 bg-black/18 p-4">
      <div className="grid gap-4 xl:grid-cols-[1fr_auto_1fr] xl:items-center">
        <FaceoffStarterCard
          ...
          align="right"
          boxScoreLine={opponentStat ? statLine(opponentStat) : null}
          highlighted={!highlightedUserWins}
          highlightLabel={winnerHighlightLabel}
        />
        <div className="w-full max-w-[360px] rounded-[22px] border border-fuchsia-200/16 bg-fuchsia-300/8 px-4 py-4 text-center xl:w-[360px]">
          <div className="text-[10px] uppercase tracking-[0.2em] text-fuchsia-100/80">
            {matchup.slot} {hasActualDuel ? "Actual Matchup" : "Projected Matchup"}
          </div>
          ...
          {hasActualDuel ? (
            <div
              className={clsx(
                "mt-2 inline-flex rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em]",
                projectionMatchedActual
                  ? "border-emerald-200/24 bg-emerald-300/12 text-emerald-100"
                  : "border-amber-200/24 bg-amber-300/12 text-amber-100",
              )}
            >
              {projectionMatchedActual ? "Projection Confirmed" : "Projection Upset"}
            </div>
          ) : (
            <div
              className={clsx(
                ...
                projectedUserWins
                  ? "border-emerald-200/24 bg-emerald-300/12 text-emerald-100"
                  : "border-rose-200/24 bg-rose-300/12 text-rose-100",
              )}
            >
              {projectedUserWins ? "Your Edge" : "Boss Edge"} / {projectedEdgeLabel}
            </div>
          )}
          {hasActualDuel ? (
            <div className="mt-3 rounded-[16px] border border-white/8 bg-black/16 px-3 py-2 text-xs leading-5 text-slate-300">
              Projected: {projectedWinnerName} favored with {matchup.ratingDelta >= 0 ? "+" : ""}
              {matchup.ratingDelta} edge / {matchup.userWinProbability}% chance.
            </div>
          ) : null}
          ...
          {!hasActualDuel ? (
            ...
            >
              ...
              {matchup.ratingDelta} projected edge / {matchup.userWinProbability}% chance
            </div>
          ) : null}
          {hasActualDuel ? (
            <div className={clsx(
              "mt-4 rounded-[18px] border px-3 py-3 text-left",
              actualUserWins
                ? "border-emerald-200/18 bg-emerald-300/8"
                : "border-rose-200/18 bg-rose-300/8",
            )}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-[10px] uppercase tracking-[0.18em] text-slate-400">Box-Score Result</div>
                  <div className="mt-1 text-sm font-semibold text-white">{actualWinnerName} had the better game</div>
                  <div className="mt-1 text-xs text-slate-300">
                    Matchup score: You {userDuelScore} / Boss {opponentDuelScore}
                  </div>
                </div>
                <div className={clsx(
                  "shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em]",
                  projectionMatchedActual
                    ? "border-emerald-200/22 bg-emerald-300/12 text-emerald-100"
                    : "border-amber-200/22 bg-amber-300/12 text-amber-100",
                )}>
                  {projectionMatchedActual ? "Confirmed" : "Upset"}
                </div>
              </div>
            </div>
          ) : null}
          <div className="mt-4 grid gap-2 rounded-[18px] border border-white/8 bg-black/16 px-3 py-3 text-left text-[11px] text-slate-200">
            {projectionReasonRows.map((row) => {
              const userWinsRow = row.userValue >= row.opponentValue;
              return (
                <div key={row.label} className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
                  <span className="uppercase tracking-[0.16em] text-slate-400">{row.label}</span>
                  <span className="h-px bg-white/8" />
                  <span className={userWinsRow ? "text-emerald-100" : "text-rose-100"}>
                    Boss {row.signed ? formatBreakdownValue(row.opponentValue) : formatBreakdownScore(row.opponentValue)} / You {row.signed ? formatBreakdownValue(row.userValue) : formatBreakdownScore(row.userValue)}
                  </span>
                </div>
              );
            })}
        ...
        <FaceoffStarterCard
          ...
          boxScoreLine={userStat ? statLine(userStat) : null}
          highlighted={highlightedUserWins}
          highlightLabel={winnerHighlightLabel}
        />
      </div>
...
const StarterRevealCard = ({
  ...
  return (
    <div
      ref={shellRef}
      onClick={() => {
        if (revealed && selectableAfterReveal) onReveal();
      }}
      className={clsx(
        "group w-full max-w-[186px] justify-self-center [perspective:1600px]",
        revealed && selectableAfterReveal && "cursor-pointer",
      )}
    >
      <div
        className={clsx(
          "relative transition-transform duration-500 [transform-style:preserve-3d]",
          revealed && selectableAfterReveal && kept && "drop-shadow-[0_0_18px_rgba(110,231,183,0.42)]",
          revealed && selectableAfterReveal && !kept && "opacity-50 grayscale",
        )}
        style={{
      ...
      >
        ...
        <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)]">
          ...
          {revealed && selectableAfterReveal ? (
            <div
              className={clsx(
                "pointer-events-none absolute left-1/2 top-3 z-10 -translate-x-1/2 rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em]",
                kept
                  ? "border-emerald-200/50 bg-emerald-300/22 text-emerald-50"
                  : "border-white/18 bg-black/58 text-slate-300",
              )}
            >
              {kept ? "Keep" : "Swap In"}
            </div>
...
const FilledRogueStarterSlotPreview = ({
  player,
  slotNumber,
  onClear,
}: {
  ...
  return (
    <button
      type="button"
      onClick={onClear}
      className="min-h-[148px] rounded-[24px] border border-amber-200/28 bg-[linear-gradient(135deg,rgba(251,191,36,0.16),rgba(9,13,24,0.86))] p-3 text-left transition hover:border-amber-100/42"
    >
      <div className="flex h-full items-center gap-3">
        <div className="h-24 w-20 shrink-0 overflow-hidden rounded-[18px] border border-white/12 bg-black/24">
          {imageUrl ? (
            <img src={imageUrl} alt={player.name} className="h-full w-full object-cover object-top" loading="lazy" referrerPolicy="no-referrer" />
          ) : (
            <div className="flex h-full w-full items-center justify-center font-display text-2xl text-white/68">
              {player.name.charAt(0)}
            </div>
          )}
        </div>
        <div className="min-w-0">
          <div className="text-[10px] uppercase tracking-[0.2em] text-amber-100/78">Starter Slot {slotNumber}</div>
          <div className="mt-1 text-lg font-semibold leading-tight text-white">{player.name}</div>
          <div className="mt-2 w-fit rounded-full border border-amber-100/18 bg-amber-200/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-100">
            {player.overall} OVR
          </div>
          <div className="mt-2 text-xs text-slate-300">Click to remove</div>
        </div>
      </div>
    </button>
  );
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        selected
          ? "border-emerald-200/36 bg-emerald-300/12 shadow-[0_16px_34px_rgba(16,185,129,0.12)]"
          : disabled
            ? "cursor-not-allowed border-white/8 bg-black/18 opacity-55"
            : "border-white/12 bg-white/6 hover:border-amber-100/26 hover:bg-amber-300/8",
      )}
    >
      <div className="flex items-center gap-3">
        <div className="h-16 w-14 shrink-0 overflow-hidden rounded-[16px] border border-white/12 bg-black/24">
          {imageUrl ? (
            <img src={imageUrl} alt={player.name} className="h-full w-full object-cover object-top" loading="lazy" referrerPolicy="no-referrer" />
          ) : (
            <div className="flex h-full w-full items-center justify-center font-display text-xl text-white/68">
              {player.name.charAt(0)}
            </div>
          )}
        </div>
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.14em]">
            <span className="rounded-full border border-white/10 bg-black/20 px-2 py-1 text-slate-200">{player.overall} OVR</span>
            <span className={clsx(
              "rounded-full border px-2 py-1",
              selected ? "border-emerald-200/22 bg-emerald-300/12 text-emerald-100" : "border-white/10 bg-white/6 text-slate-300",
            )}>
              {selected ? "Slotted" : disabled ? "Slots full" : "Add"}
            </span>
          </div>
        </div>
      </div>
...
const NaturalPositionPlayerOption = ({
  player,
  selected,
  onSelect,
}: {
  ...
  const longestNameLineLength = nameLines.reduce((max, line) => Math.max(max, line.length), 0);
  const nameClassName =
    longestNameLineLength >= 13
      ? "text-[0.56rem]"
      : longestNameLineLength >= 11
        ? "text-[0.64rem]"
        : longestNameLineLength >= 9
          ? "text-[0.72rem]"
          : longestNameLineLength >= 8
            ? "text-[0.78rem]"
            : longestNameLineLength >= 7
              ? "text-[0.86rem]"
              : "text-[1rem]";
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={clsx(
        "group rounded-[30px] border border-dashed p-1 text-left transition hover:-translate-y-0.5",
        selected
          ? "border-sky-200/70 shadow-[0_0_0_1px_rgba(125,211,252,0.28),0_0_30px_rgba(56,189,248,0.22)]"
          : "border-white/32 hover:border-white/54",
      )}
    >
      <div
        className={clsx(
          "relative overflow-hidden rounded-[26px] border px-4 py-4 shadow-[0_18px_42px_rgba(0,0,0,0.26)]",
          selected ? "border-sky-200/50" : "border-white/12",
          playerTierRunRosterSurfaceStyles[tier],
        )}
      >
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.13),transparent_32%),linear-gradient(90deg,rgba(2,6,23,0.34),rgba(2,6,23,0.04)_48%,rgba(2,6,23,0.46)),linear-gradient(180deg,transparent,rgba(2,6,23,0.42))]" />
        <div className="relative grid min-h-[104px] grid-cols-[80px_minmax(0,1fr)_64px] items-center gap-3">
          <div className="h-20 w-20 shrink-0 overflow-hidden rounded-[20px] border border-white/12 bg-black/26 shadow-[0_12px_26px_rgba(0,0,0,0.28)]">
            {imageUrl ? (
              <img
                src={imageUrl}
                alt={player.name}
          ...
          <div className="min-w-0 flex-1">
            <div className="min-w-0 rounded-[20px] border border-white/12 bg-[linear-gradient(180deg,rgba(4,8,18,0.68),rgba(4,8,18,0.86))] px-3 py-3 shadow-[0_12px_26px_rgba(0,0,0,0.28)] backdrop-blur-[4px]">
              <div className="text-[10px] font-semibold uppercase tracking-[0.24em] text-slate-400">
                {player.primaryPosition}
              </div>
              <div
                className={clsx(
                  "mt-1 min-w-0 font-display font-semibold leading-[1.02] text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.55)]",
                  nameClassName,
                )}
              >
                {nameLines.map((line) => (
              ...
              <div className="mt-2 break-words text-[0.68rem] font-semibold uppercase leading-snug tracking-[0.15em] text-slate-300">
                {naturalPositions}
              </div>
            </div>
          </div>

          <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-[20px] border border-white/18 bg-[linear-gradient(180deg,rgba(4,8,18,0.72),rgba(4,8,18,0.9))] text-white shadow-[0_12px_26px_rgba(0,0,0,0.28)] backdrop-blur-[4px]">
            <div className="font-display text-2xl font-semibold leading-none">{player.overall}</div>
            <div className="mt-1 text-[9px] font-bold uppercase leading-none tracking-[0.16em] text-slate-300">OVR</div>
          </div>
        </div>
      </div>
    </button>
...
const FinalVictoryStarterCard = ({
  ...
  return (
    <div className="overflow-hidden rounded-[18px] border border-white/12 bg-[linear-gradient(180deg,rgba(6,10,19,0.88),rgba(15,20,34,0.96))] shadow-[0_18px_44px_rgba(0,0,0,0.24)]">
      <div className="relative h-36 overflow-hidden bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.12),transparent_48%),linear-gradient(180deg,rgba(18,25,40,0.96),rgba(5,8,16,0.98))]">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/88 via-black/50 to-transparent px-3 pb-3 pt-10">
          <div className="text-center text-[0.98rem] font-semibold leading-tight text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]">
            {displayPlayer.name}
          </div>
        </div>
      </div>
...
export const RoguelikeMode = ({
  ...
  const [chanceWheelSpinning, setChanceWheelSpinning] = useState(false);
  const [chanceWheelRotation, setChanceWheelRotation] = useState(0);
  ...
  const appliedChallengeSetupRequestRef = useRef<number | null>(null);
  const trackedBossReachedKeysRef = useRef<Set<string>>(new Set());
  const trackedRunEndKeysRef = useRef<Set<string>>(new Set());
  ...
  const recordRogueChallengesForClearedNode = (sourceRun: RoguelikeRun, node: RoguelikeNode) => {
    if (node.type !== "boss") return;

    const startingLineup = sourceRun.lineup
      .slice(0, 5)
      .map((slot) => slot.player)
      .filter((player): player is Player => Boolean(player));
  ...
  useEffect(() => {
    if (!challengeSetupRequest) return;
    if (appliedChallengeSetupRequestRef.current === challengeSetupRequest.requestId) return;

    appliedChallengeSetupRequestRef.current = challengeSetupRequest.requestId;
    setSelectedRunSettings((currentSettings) => ({
      ...currentSettings,
      ...challengeSetupRequest.settings,
    }));
    setShowRunSettingsScreen(true);
    setShowPackSelectionHub(Boolean(run));
  ...
  useEffect(() => {
    ...
    onUpdatePersonalBests({
      ...
      hardestDifficulty: run.settings.difficulty,
      hardestDifficultyFloor: furthestFloor,
    });
  }, [
    run,
    runNodes.length,
  ...
  useEffect(() => {
    ...
    if (run.stage === "faceoff-setup" && run.activeNode?.type === "boss") {
      const bossKey = `${run.seed}-${run.activeNode.id}-${run.floorIndex}`;
      if (!trackedBossReachedKeysRef.current.has(bossKey)) {
        trackedBossReachedKeysRef.current.add(bossKey);
        trackAnalyticsEventSoon("boss_reached", {
          runId: run.seed,
          payload: {
            floorIndex: run.floorIndex,
            floorNumber: run.floorIndex + 1,
            bossId: run.activeNode.id,
            bossTitle: run.activeNode.title,
            opponentTeamName: run.activeNode.opponentTeamName,
    ...
    if (run.stage === "run-cleared" || run.stage === "run-over") {
      const runEndKey = `${run.seed}-${run.stage}`;
      if (!trackedRunEndKeysRef.current.has(runEndKey)) {
        trackedRunEndKeysRef.current.add(runEndKey);
        trackAnalyticsEventSoon(run.stage === "run-cleared" ? "run_completed" : "run_failed", {
          runId: run.seed,
          payload: {
            floorIndex: run.floorIndex,
            floorNumber: run.floorIndex + 1,
            ...
            lives: run.lives,
            resultTitle: run.nodeResult?.title,
            resultPassed: run.nodeResult?.passed,
          },
        });
      }
    }
    ...
    run?.activeNode?.id,
    run?.activeNode?.opponentTeamName,
    run?.activeNode?.title,
    run?.activeNode?.type,
    run?.floorIndex,
    run?.lives,
  ...
  const startRun = (
    ...
    trackAnalyticsEventSoon("starter_pack_selected", {
      runId: seed,
      payload: {
        packageId,
        starterPackUpgrade: selectedStarterPackUpgrade,
    ...
    trackAnalyticsEventSoon("rogue_run_started", {
      runId: seed,
      payload: {
        packageId,
        starterPackUpgrade: selectedStarterPackUpgrade,
        difficulty: runSettings.difficulty,
        enableCoaches: runSettings.enableCoaches,
        rogueStarterSlotIds: selectedRogueStarterSlotIds.filter((playerId): playerId is string => Boolean(playerId)),
      },
    });
    ...
    const starterRevealSource = playerUniverse.filter((player) => !selectedRogueStarterIdSetForRun.has(player.id));
    const baseStarterRevealPlayers = ensureVisibleStarterRevealPlayers(
      packageId,
      seed,
    ...
    const starterRevealPlayersWithRogueStars = injectRogueStarterSlotsIntoReveal(
      baseStarterRevealPlayers,
      selectedRogueStarterPlayersForRun,
    );
    const starterRevealPlayers =
      ownedStarterPackChoicePlus > 0
        ? addStarterChoicePlusOption(
            starterRevealPlayersWithRogueStars,
            seed,
            getStarterPackAverageForUpgrade(selectedStarterPackUpgrade),
            null,
            starterRevealSource,
    ...
    setRun({
      ...
      floorIndex: 0,
        initialPicks: 0,
        draftShuffleTickets: ownedExtraDraftShuffle > 0 ? 1 : 0,
        lockerRoomCash: getOpeningLockerCashForTier(ownedOpeningLockerCashTier),
        unlockedBundleIds: [],
      scoutedBossNodeIds: [],
  ...
  const revealStarterCard = (playerId: string) => {
    if (!run || run.stage !== "starter-reveal") return;
    const choicePlusActive = run.starterRevealPlayers.length > 3;
    const allStarterCardsRevealed = run.revealedStarterIds.length === run.starterRevealPlayers.length;

    if (run.revealedStarterIds.includes(playerId)) {
      if (!choicePlusActive || !allStarterCardsRevealed) return;

      const currentKeptIds = run.starterKeptIds ?? run.revealedStarterIds.slice(0, 3);
      const nextStarterKeptIds = currentKeptIds.includes(playerId)
        ? currentKeptIds.filter((keptPlayerId) => keptPlayerId !== playerId)
        : currentKeptIds.length < 3
          ? [...currentKeptIds, playerId]
          : currentKeptIds;
      const revealedStarterRosterState = buildRevealedStarterRosterState(run, nextStarterKeptIds);

      setRun({
        ...run,
        ...revealedStarterRosterState,
        starterKeptIds: nextStarterKeptIds,
      });
      return;
    }

    const nextRevealedStarterIds = [...run.revealedStarterIds, playerId];
    const nextAllStarterCardsRevealed = nextRevealedStarterIds.length === run.starterRevealPlayers.length;
    const nextStarterKeptIds = choicePlusActive
      ? nextAllStarterCardsRevealed
        ? nextRevealedStarterIds.slice(0, 3)
        : run.starterKeptIds ?? []
      : nextRevealedStarterIds;
    const revealedStarterRosterState = buildRevealedStarterRosterState(
      run,
      choicePlusActive ? nextStarterKeptIds : nextRevealedStarterIds,
    );
    setRun({
      ...run,
      ...revealedStarterRosterState,
  ...
  const proceedToRunLadder = () => {
    if (!run || run.stage !== "starter-reveal") return;
    const choicePlusActive = run.starterRevealPlayers.length > 3;
    if (choicePlusActive && (run.starterKeptIds?.length ?? 0) !== 3) return;
    const starterKeptIds = choicePlusActive ? run.starterKeptIds ?? [] : run.revealedStarterIds;
    const starterRosterState = buildRevealedStarterRosterState(run, starterKeptIds);
    const hydratedRun = { ...run, ...starterRosterState };
    const { ownedPlayers, lineup } = getEarlyRunRosterState(hydratedRun);
  ...
  const hireCoach = (coachId: string) => {
    ...
    trackAnalyticsEventSoon("coach_selected", {
      runId: run.seed,
      payload: {
        coachId,
        coachName: selectedCoach?.name,
        coachTeamName: selectedCoach?.teamName,
        floorIndex: run.floorIndex,
        difficulty: run.settings.difficulty,
      },
    });
  ...
  const startOpeningDraft = () => {
    ...
    if (run.activeNode?.id === currentNode.id) {
      ...
      if (currentNode.type === "chance") {
        setRun({ ...run, stage: "chance-spin", nodeResult: null });
        return;
      }
      ...
      if (currentNode.battleMode === "starting-five-faceoff") {
        ...
        setRun({
          ...
          activeOpponentPlayerIds:
            ...
            generateFaceoffOpponentPlayerIds(
              getRunOwnedPlayers(hydratedRun),
      ...
      if (evolutionOptions.length === 0) {
        ...
        setRun({
          ...
          nodeResult: {
            ...
            detail: "No eligible version player was on your roster, so this floor converted into 1 Draft Shuffle ticket instead. You can use it later to reroll a live five-player board.",
            passed: true,
          },
        });
        return;
  ...
  const spinChanceWheel = () => {
    if (!run || run.stage !== "chance-spin" || run.activeNode?.type !== "chance") return;
    if (chanceWheelSpinning) return;

    const node = run.activeNode;
    const landedTeamName = getChanceWheelTeamName(run, node);
    const landedTeamIndex = Math.max(0, CHANCE_WHEEL_TEAMS.indexOf(landedTeamName));
    const teamPool = getChanceWheelPlayerPool(landedTeamName, run.availablePool, runPlayerUniverse);
    const nextChoicesState = drawRunChoices(
      run,
      teamPool,
      getRunOwnedPlayers(run),
      node.rewardChoices,
      nextChoiceSeed(run.seed, 740 + run.floorIndex * 43 + landedTeamIndex),
      getNodeChoiceTiers(node) ? [...getNodeChoiceTiers(node)!] : undefined,
      true,
      false,
    );
    const landedTeam = getNbaTeamByName(landedTeamName);
    const tierLabel = getChanceWheelTierLabel(node);
    const choiceCount = nextChoicesState.choices.length;
    ...
    window.setTimeout(() => {
      setChanceWheelSpinning(false);
      setRun({
        ...run,
        chanceWheelTeamName: landedTeamName,
        seenChoicePlayerIds: nextChoicesState.seenChoicePlayerIds,
        choices: nextChoicesState.choices,
        stage: "reward-draft",
        nodeResult: {
          title: `${landedTeam?.abbreviation.toUpperCase() ?? landedTeamName} board unlocked`,
          detail:
            choiceCount > 0
              ? `The Team Wheel landed on ${landedTeamName}. Choose 1 of ${choiceCount} ${tierLabel} players from that team for your run roster.`
              : `The Team Wheel landed on ${landedTeamName}, but no eligible ${tierLabel} players are available with the current run settings. You can skip this pick and continue the climb.`,
          passed: true,
        },
      });
    }, CHANCE_WHEEL_SPIN_DURATION_MS);
  };
  ...
  const continueAfterFaceoff = () => {
    ...
    if (node.type === "boss") {
      trackAnalyticsEventSoon(passed ? "boss_won" : "boss_lost", {
        runId: run.seed,
        payload: {
          floorIndex: run.floorIndex,
          floorNumber: run.floorIndex + 1,
          bossId: node.id,
          bossTitle: node.title,
          opponentTeamName: node.opponentTeamName,
          difficulty: run.settings.difficulty,
          userTeamWinProbability: run.nodeResult.faceoffResult?.userTeamWinProbability,
    ...
    recordRogueChallengesForClearedNode(run, node);
    ...
    if ((node.draftShuffleReward ?? 0) > 0 && node.rewardChoices === 0) {
      ...
      setRun({
        ...
        nodeResult: {
          title: `${node.title} cleared`,
          detail: `You beat ${node.opponentTeamName ?? "the boss team"} and earned ${rewardAmount} Draft Shuffle ticket${rewardAmount > 1 ? "s" : ""}. Your next floor is ready.`,
          passed: true,
          faceoffResult: run.nodeResult.faceoffResult ?? null,
        },
      });
      ...
      nodeResult: {
        ...
        detail:
          node.id === "act-one-faceoff" || node.id === "act-one-faceoff-current"
              ...
              : node.id === "act-one-boss" || node.id === "act-one-boss-current"
                ? `You beat ${node.opponentTeamName ?? "the boss team"}. Choose 1 of 3 Ruby version players now, then look for a future evolution floor to upgrade that player into a higher-rated version.`
  ...
  const buyLockerRoomSpecialStuff = () => {
    ...
    setRun({
      ...
      lockerRoomNotice: {
        ...
        detail: `You banked Special Stuff for this run. Save it for any future boss matchup, then apply it to give one player up to +${SPECIAL_STUFF_BOOST_AMOUNT} OVR for that boss game only.`,
      },
    });
  };
  ...
  const sendPlayerToTraining = (player: Player) => {
    ...
    const ownedPlayers = getRunOwnedPlayers(run);
    const sourcePlayer = ownedPlayers.find((ownedPlayer) => ownedPlayer.id === player.id) ?? player;
    const overallBoostAmount = getOverallBoostAmountForTrainingNode(run.activeNode);
    const ownedPlayerIds = ownedPlayers.map((ownedPlayer) => ownedPlayer.id);
    const coachTeamKey = getRoguelikeCoachTeamKey(run.hiredCoachId);
    const isSpecialStuffSelection = run.activeNode.id === LOCKER_ROOM_SPECIAL_STUFF_NODE.id;

    if (
      overallBoostAmount > 0 &&
      !isSpecialStuffSelection &&
      !canApplyRunOverallBoost(sourcePlayer, overallBoostAmount, ownedPlayerIds, run.trainedPlayerIds ?? [], coachTeamKey)
    ) {
      return;
    }
    ...
    if (isSpecialStuffSelection) {
      ...
      if (specialStuffBoostAmount <= 0) return;
      ...
      setRun(restoreUtilityReturnState({
        ...
        lockerRoomNotice: {
          ...
          detail: `${player.name} will get +${specialStuffBoostAmount} OVR for ${nextBossNode?.title ?? "the selected boss matchup"} only.`,
        },
    ...
    setRun({
      ...
      nodeResult: {
        ...
        detail: `${player.name} went to training and now has +${trainingCount} total training OVR applied for the rest of this run. ${nextNode ? "Your next floor is ready." : ""}`,
        passed: true,
      },
    });
  };
  ...
  const declineTradeDeadline = () => {
    ...
    setRun({
      ...
      nodeResult: {
        title: `${run.activeNode.title} complete`,
        detail: `You stood pat at the deadline and kept your roster intact. ${nextNode ? "The next floor is ready." : ""}`,
        passed: true,
      },
    });
  };
  ...
  const useDraftShuffleTicket = () => {
    ...
    if (run.stage === "reward-draft" && run.pendingTradeState && sourceNode) {
      ...
      const tradeReplacementMode = run.pendingTradeState.replacementMode ?? "similar";
      const tradeReplacementPool =
        tradeReplacementMode === "fit" && run.pendingTradeState.outgoingPlayerTier
          ? getTradeForFitPool(
              run.availablePool,
              runPlayerUniverse,
              run.pendingTradeState.outgoingPlayerTier,
              run.roster,
      ...
      let nextChoices =
        tradeReplacementMode === "fit"
          ? drawTradeForFitChoices(
              tradeReplacementPool,
              run.roster,
  ...
  const tradePlayerForReplacement = (player: Player) => {
    ...
    setRun({
      ...run,
      choices: [],
      stage: "trade-path-select",
      pendingTradeState: {
        outgoingPlayerId: player.id,
        outgoingPlayerOverall: tradedOverall,
        outgoingPlayerName: baseTradedPlayer.name,
        outgoingPlayerTier: getPlayerTier(baseTradedPlayer),
  ...
  const chooseTradeReplacementMode = (replacementMode: TradeReplacementMode) => {
    if (!run?.activeNode || run.stage !== "trade-path-select" || !run.pendingTradeState) return;

    const sourceNode = run.activeNode;
    const coachTeamKey = getRoguelikeCoachTeamKey(run.hiredCoachId);
    const pendingTrade = run.pendingTradeState;
    const nextRoster = pendingTrade.originalRoster.filter((owned) => owned.id !== pendingTrade.outgoingPlayerId);
    const nextLineup = pendingTrade.originalLineup.map((slot) =>
      slot.player?.id === pendingTrade.outgoingPlayerId ? { ...slot, player: null } : { ...slot },
    );
    const replacementChoiceCount = getTradeReplacementChoiceCount(sourceNode, run.pendingChoiceSelection);
    const tradeReplacementPool =
      replacementMode === "fit" && pendingTrade.outgoingPlayerTier
        ? getTradeForFitPool(
            run.availablePool,
            runPlayerUniverse,
            pendingTrade.outgoingPlayerTier,
            nextRoster,
            pendingTrade.outgoingPlayerId,
            coachTeamKey,
          )
        : getSimilarCaliberTradePool(
            sourceNode,
            run.availablePool,
            runPlayerUniverse,
            pendingTrade.outgoingPlayerOverall,
            nextRoster,
            pendingTrade.outgoingPlayerId,
            coachTeamKey,
          );
    ...
    const replacementFloor = pendingTrade.outgoingPlayerOverall - 1;
    const replacementCeiling = pendingTrade.outgoingPlayerOverall + 1;
    const outgoingName = pendingTrade.outgoingPlayerName ?? "Your player";
    ...
    setRun({
      ...
      nodeResult: {
        ...
        detail:
          replacementMode === "fit"
            ? `${outgoingName} is on the trade block. Choose 1 lower-tier connected player who links to your current roster or coach, or skip this board to keep ${outgoingName}.`
            : `${outgoingName} is on the trade block. Choose 1 of ${nextChoices.length} similar-caliber replacement players rated between ${replacementFloor} and ${replacementCeiling} OVR, or skip this board to keep ${outgoingName}.`,
        passed: true,
      },
    });
  };
  ...
  useEffect(() => {
    ...
    const repairedPool =
      isTradeReplacementBoard && run.pendingTradeState
        ? run.pendingTradeState.replacementMode === "fit" && run.pendingTradeState.outgoingPlayerTier
          ? getTradeForFitPool(
              run.availablePool,
              runPlayerUniverse,
              run.pendingTradeState.outgoingPlayerTier,
    ...
    const repairedChoicesState =
      isTradeReplacementBoard && run.pendingTradeState
        ? (() => {
              const repairedTradeChoices =
                run.pendingTradeState?.replacementMode === "fit"
                  ? drawTradeForFitChoices(
                      repairedPool,
                      ownedPlayers,
        ...
        : drawRunChoices(
            ...
            sourceNode.type === "chance" ? true : shouldStrictlyUseNodePool(sourceNode),
  ...
  const backToRunLadder = () => {
    ...
    const isDraftBoardStage =
      run.stage === "initial-draft" ||
      run.stage === "reward-draft" ||
      run.stage === "reward-replace-select";
    const shouldPreserveDraftBoard =
      isDraftBoardStage &&
      !run.pendingTradeState &&
      (run.choices.length > 0 || Boolean(run.pendingRewardPlayer));
    const restoredTradeRun =
      run.pendingTradeState && (run.stage === "trade-path-select" || run.stage === "reward-draft")
        ? {
            ...run,
            roster: run.pendingTradeState.originalRoster,
            lineup: run.pendingTradeState.originalLineup,
          }
        : run;
    const currentNode = runNodes[restoredTradeRun.floorIndex] ?? null;
    const activeNode = shouldPreserveDraftBoard
      ? restoredTradeRun.activeNode ?? currentNode
      : restoredTradeRun.activeNode;
    ...
    setRun({
      ...
      stage: "ladder-overview",
      activeNode,
      choices: shouldPreserveDraftBoard ? restoredTradeRun.choices : [],
      selectedCutPlayerIds: [],
      selectedNaturalPositionPlayerId: null,
      selectedNaturalPosition: null,
      pendingChoiceSelection: null,
  ...
  const assignRogueStarterSlot = (slotIndex: number, playerId: string | null) => {
    setSelectedRogueStarterSlotIds((currentSlots) => {
      ...
      if (playerId && nextSlots.includes(playerId)) {
        return nextSlots.map((currentPlayerId) => currentPlayerId === playerId ? null : currentPlayerId);
      }
  ...
  const toggleRogueStarterPlayer = (playerId: string) => {
    setSelectedRogueStarterSlotIds((currentSlots) => {
      if (currentSlots.includes(playerId)) {
        return currentSlots.map((currentPlayerId) => currentPlayerId === playerId ? null : currentPlayerId);
      }

      const emptySlotIndex = currentSlots.findIndex((currentPlayerId) => currentPlayerId === null);
      if (emptySlotIndex === -1) return currentSlots;

      const nextSlots = [...currentSlots];
      nextSlots[emptySlotIndex] = playerId;
      return nextSlots;
    });
  };
  const clearRogueStarterSlots = () => setSelectedRogueStarterSlotIds([null, null, null]);
                  ...
                  {ROGUE_DIFFICULTY_OPTIONS.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={clsx(
                            "mt-3 w-fit rounded-full border px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.14em]",
                            selected
                              ? "border-amber-100/28 bg-amber-200/14 text-amber-50"
                              : "border-amber-200/14 bg-amber-300/8 text-amber-100/82",
                          )}
                        >
                ...
                {[
                  {
                    ...
                    title: "Galaxy Cards",
                    detail: "Galaxy cards will appear in the run.",
                    icon: Sparkles,
                    invertValue: true,
                  },
                  {
                  ...
                  },
                  {
                    ...
                    detail: "Training camp floors will appear in the ladder.",
                    icon: Zap,
                    invertValue: true,
                  },
                  {
                    ...
                    title: "Trades",
                    detail: "Trade floors will appear in the ladder.",
                    icon: RefreshCcw,
                    invertValue: true,
                  },
                ].map((toggle) => {
                  const enabled = toggle.invertValue
                    ? !selectedRunSettings[toggle.key]
                    : selectedRunSettings[toggle.key];
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        updateRunSetting(toggle.key, toggle.invertValue ? !nextEnabled : nextEnabled);
                      }}
                      className={clsx(
          ...
          <div className="glass-panel rounded-[30px] border border-white/14 bg-[linear-gradient(180deg,rgba(15,23,42,0.82),rgba(6,10,18,0.94))] p-5 shadow-[0_24px_70px_rgba(0,0,0,0.42),inset_0_1px_0_rgba(255,255,255,0.05)] lg:p-6">
            ...
            <div className="mt-6 flex flex-col gap-3">
              <button
                ...
                className="group relative inline-flex items-center justify-center gap-3 overflow-hidden rounded-full border border-amber-100/70 bg-[linear-gradient(135deg,#ffffff,#fef3c7,#f59e0b)] px-7 py-4 text-sm font-extrabold uppercase tracking-[0.12em] text-slate-950 shadow-[0_0_0_1px_rgba(251,191,36,0.35),0_0_30px_rgba(251,191,36,0.42),0_18px_34px_rgba(0,0,0,0.32)] transition hover:scale-[1.025] hover:shadow-[0_0_0_1px_rgba(254,243,199,0.65),0_0_44px_rgba(251,191,36,0.6),0_20px_38px_rgba(0,0,0,0.34)]"
              >
                ...
                <span className="relative">Continue to Starter Packs</span>
                <ArrowRight size={17} className="relative transition group-hover:translate-x-0.5" />
              </button>
              <button
                type="button"
                  onClick={() => {
  ...
  if (!run || showPackSelectionHub) {
    return (
      <section className="space-y-3">
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_500px] xl:items-start">
            <div className="min-w-0">
              ...
              <p className="mt-2.5 max-w-4xl text-sm leading-6 text-slate-200/85">
                This new mode is run-based. You start with a small opening roster, draft just a few cards, then survive a ladder of draft floors, chemistry checks, and boss gates while pulling from the full eligible player pool all run long.
              </p>
            </div>
        ...
        <div className="glass-panel rounded-[30px] border border-amber-200/14 bg-[linear-gradient(135deg,rgba(80,53,13,0.32),rgba(9,13,24,0.9))] p-4 shadow-card lg:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-xs uppercase tracking-[0.24em] text-amber-100/74">
                <Crown size={15} />
                Owned Starter Cards
              </div>
            ...
            {selectedRogueStarterPlayers.length > 0 ? (
              <button
                type="button"
                onClick={clearRogueStarterSlots}
                className="w-fit rounded-full border border-white/12 bg-white/6 px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-white transition hover:bg-white/10"
              >
                Clear Slots
              </button>
            ) : null}
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {selectedRogueStarterSlotIds.map((playerId, index) => (
              <RogueStarterSlotPreview
                key={`rogue-starter-slot-${index}`}
  ...
  if (run.stage === "starter-reveal") {
    const allStarterCardsRevealed = run.revealedStarterIds.length === run.starterRevealPlayers.length;
    const starterChoicePlusActive = run.starterRevealPlayers.length > 3;
    const keptStarterCount = run.starterKeptIds?.length ?? (starterChoicePlusActive ? 0 : run.revealedStarterIds.length);
    const canProceedFromStarterReveal =
      ...
      <section className="space-y-3">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
                {starterChoicePlusActive
                  ? "Open the four starter cards from your selected pack, then keep the three players you want for this Rogue run."
        ...
        <div className="rounded-[26px] border border-white/14 bg-[linear-gradient(180deg,rgba(10,14,20,0.98),rgba(16,22,32,0.99))] px-5 py-4 shadow-card">
          ...
          <h2 className="mt-1 font-display text-2xl text-white">
            {starterChoicePlusActive ? "Open 4 cards, keep 3" : "Open your first 3 cards"}
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
            {starterChoicePlusActive
              ? allStarterCardsRevealed
                ? `Click revealed cards to set your keepers. ${keptStarterCount}/3 selected.`
                : "Reveal all four cards, then choose the three players who will start this run."
              : "Reveal all three to see which players are starting this run before you move on to the Run Ladder."}
          </p>
          <div data-tutorial-id="rogue-starter-reveal-cards" className="mt-5 grid grid-cols-4 gap-2 sm:flex sm:flex-wrap sm:justify-center sm:gap-5">
            {run.starterRevealPlayers.map((player, index) => {
              const revealed = run.revealedStarterIds.includes(player.id);
              const kept = !starterChoicePlusActive || (run.starterKeptIds ?? []).includes(player.id);
              ...
              return (
                <StarterRevealCard
                  ...
                  revealed={revealed}
                  kept={kept}
                  selectableAfterReveal={starterChoicePlusActive && allStarterCardsRevealed}
  ...
  const runOwnedPlayerById = new Map(runOwnedPlayers.map((player) => [player.id, player]));
  ...
  const activeSpecialStuffPlayer = run.activeSpecialStuffPlayerId
    ? runOwnedPlayers.find((player) => player.id === run.activeSpecialStuffPlayerId) ?? null
    : null;
  const activeSpecialStuffBoostAmount = activeSpecialStuffPlayer
    ? getSpecialStuffBoostAmountForPlayer(
        activeSpecialStuffPlayer,
        runOwnedPlayerIds,
        run.trainedPlayerIds ?? [],
        runCoachTeamKey,
      )
  ...
  const draftChoiceRevealKey = [
    ...
    run.pendingTradeState?.replacementMode ?? "standard",
  ...
  const nodeResultNextStepDescription = nodeResultHasRewardChoices
    ...
    : upcomingNodeAfterResult
      ...
      : "This floor is complete. Continue when you're ready for the next step.";
  ...
  const pendingTrade = run.pendingTradeState;
  const pendingTradeNextRoster =
    pendingTrade?.originalRoster.filter((player) => player.id !== pendingTrade.outgoingPlayerId) ?? [];
  const tradeForFitEnabled =
    Boolean(activeNode && pendingTrade?.outgoingPlayerTier) &&
    canNodeOfferTradeForFit(activeNode, runNodes, run.pendingChoiceSelection);
  const tradeForFitPool =
    tradeForFitEnabled && pendingTrade?.outgoingPlayerTier
      ? getTradeForFitPool(
          run.availablePool,
          runPlayerUniverse,
          pendingTrade.outgoingPlayerTier,
          pendingTradeNextRoster,
          pendingTrade.outgoingPlayerId,
          runCoachTeamKey,
        )
      : [];
  const tradeForFitChoiceCount = activeNode
    ? getTradeReplacementChoiceCount(activeNode, run.pendingChoiceSelection)
    : 0;
  const canTradeForFit = tradeForFitEnabled && tradeForFitPool.length > 0;
      ...
      >
        <div className="space-y-6">
          ...
          {run.stage === "ladder-overview" && (
            <div className="glass-panel rounded-[30px] p-6 shadow-card">
              ...
              <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-300">
                This is the full run path. You will move through draft floors, chemistry checks, and boss gates until you either survive the gauntlet or the run collapses. Study the ladder first, then begin your opening draft.
              </p>
                  ...
                  <div className={clsx("mt-4 grid", isMobileViewport ? "grid-cols-2 gap-3" : "gap-4 xl:grid-cols-3")}>
                        ...
                        meta: run.activeSpecialStuffPlayerId
                          ...
                          : run.specialStuffInventoryCount > 0
                            ...
                            : nextBossNode
                              ? "Need purchase"
                              : "No boss",
                        description: `Buy now, save it for any future boss matchup this run. When used, 1 player gets up to +${SPECIAL_STUFF_BOOST_AMOUNT} OVR for that boss game only.`,
                        detail: run.activeSpecialStuffPlayerId
                    ...
                    <div className="rounded-[28px] border border-cyan-200/18 bg-[linear-gradient(135deg,rgba(14,116,144,0.24),rgba(7,20,32,0.94))] p-5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="mt-3 text-sm leading-7 text-slate-300">
                          Buy now, save it for any future boss matchup this run. When used, 1 player gets up to +{SPECIAL_STUFF_BOOST_AMOUNT} OVR for that boss game only.
                        </div>
                      </div>
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[22px] border border-cyan-200/18 bg-cyan-300/8 px-4 py-4 text-sm text-slate-100">
                  <div>
                    ...
                    <div className="mt-1 text-slate-300">
                      {run.activeSpecialStuffPlayerId
                        ? `${getPlayerById(run.activeSpecialStuffPlayerId)?.name ?? "Selected player"} is boosted +${activeSpecialStuffBoostAmount} OVR for this boss game.`
                        : `${run.specialStuffInventoryCount} owned. Apply one before tipoff to give a player up to +${SPECIAL_STUFF_BOOST_AMOUNT} OVR for this boss game.`}
                    </div>
                  </div>
                  <button
                    type="button"
          ...
          {run.stage === "choice-select" && activeNode && (
            <div className="glass-panel rounded-[30px] p-6 shadow-card">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Choice Floor</div>
              <h2 className="mt-2 font-display text-3xl text-white">{activeNode.title}</h2>
              <p className="mt-3 text-sm leading-7 text-slate-300">
            ...
            <div className="glass-panel rounded-[30px] p-6 shadow-card">
              ...
              <div className="mt-5 flex flex-wrap items-start justify-center gap-2 overflow-hidden">
                {trainingSelectionDisplayPlayers.map((player) => {
                  ...
                  const isSpecialStuffSelection = activeNode.id === LOCKER_ROOM_SPECIAL_STUFF_NODE.id;
                  const specialStuffBoostAmount = isSpecialStuffSelection
                    ? getSpecialStuffBoostAmountForPlayer(
                        sourcePlayer,
                        runOwnedPlayerIds,
                        run.trainedPlayerIds ?? [],
                    ...
                    : 0;
                  const trainingWouldExceedCap =
                    activeTrainingOverallBoostAmount > 0 &&
                    (isSpecialStuffSelection
                      ? specialStuffBoostAmount <= 0
                      : !canApplyRunOverallBoost(
                          sourcePlayer,
                          activeTrainingOverallBoostAmount,
                          runOwnedPlayerIds,
                          run.trainedPlayerIds ?? [],
                          runCoachTeamKey,
                        ));
                  ...
                  return (
                    <div key={player.id} className="flex flex-col items-center gap-2">
                      <DraftPlayerCard
                        ...
                        disabled={trainingWouldExceedCap}
                        compact
                      ...
                      {trainingWouldExceedCap ? (
                        ...
                        <div className="max-w-[180px] rounded-full border border-cyan-200/18 bg-cyan-300/10 px-3 py-1 text-center text-[10px] font-semibold uppercase tracking-[0.14em] text-cyan-100">
                          +{specialStuffBoostAmount} this boss
                        </div>
                      ) : null}
                    </div>
                  );
              ...
              <div className="mt-6">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {runOwnedDisplayPlayers.map((player) => {
                    ...
                    return (
                      <NaturalPositionPlayerOption
                        key={player.id}
                        player={player}
                        selected={selected}
                        onSelect={() => selectNaturalPositionPlayer(player)}
                      />
                    );
                  })}
                </div>
          ...
          {run.stage === "trade-offer" && activeNode && (
            <div className="glass-panel rounded-[30px] p-6 shadow-card">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-400">{isChoiceTradePath ? "Choice Floor" : "Trade Floor"}</div>
          ...
          {run.stage === "trade-path-select" && activeNode && pendingTrade && (
            <div className="glass-panel rounded-[30px] p-6 shadow-card">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-400">{isChoiceTradePath ? "Choice Floor" : "Trade Floor"}</div>
              <h2 className="mt-2 font-display text-3xl text-white">Choose Your Trade Path</h2>
              <p className="mt-3 text-sm leading-7 text-slate-300">
                {pendingTrade.outgoingPlayerName ?? "Your player"} is on the block. Keep the current similar-value market, or sacrifice tier value to hunt for a lower-tier player with a real chemistry connection.
              </p>
              <div className="mt-5 grid gap-3 lg:grid-cols-2">
                <button
                  type="button"
                  onClick={() => chooseTradeReplacementMode("similar")}
                  className="group rounded-[22px] border border-white/12 bg-white/6 p-4 text-left transition hover:border-white/24 hover:bg-white/10"
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.2em] text-slate-400">Current Trade</div>
                      <div className="mt-1 text-xl font-semibold text-white">Similar Value</div>
                    </div>
                    <ArrowRight size={18} className="text-white/70 transition group-hover:translate-x-0.5" />
                  </div>
                  <p className="mt-3 text-sm leading-6 text-slate-300">
                    Get a replacement board within 1 OVR of the outgoing player. Best when you want to reshape without giving up power.
                  </p>
                </button>

                <button
                  type="button"
                  onClick={() => chooseTradeReplacementMode("fit")}
                  disabled={!canTradeForFit}
                  className={clsx(
                    "group rounded-[22px] border p-4 text-left transition",
                    canTradeForFit
                      ? "border-emerald-200/22 bg-emerald-300/10 hover:border-emerald-100/34 hover:bg-emerald-300/14"
                      : "cursor-not-allowed border-white/8 bg-white/[0.03] opacity-55",
                  )}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.2em] text-emerald-100/76">Chemistry Market</div>
                      <div className="mt-1 text-xl font-semibold text-white">Trade For Fit</div>
                    </div>
                    <Sparkles size={18} className={canTradeForFit ? "text-emerald-100" : "text-slate-500"} />
                  </div>
                  <p className="mt-3 text-sm leading-6 text-slate-300">
                    Choose from lower-tier players who connect to your roster or coach. Great for team links, badge paths, and challenge builds.
                  </p>
                  <div className="mt-3 rounded-2xl border border-white/10 bg-black/18 px-3 py-2 text-xs text-slate-200">
                    {canTradeForFit
                      ? `${Math.min(tradeForFitChoiceCount, tradeForFitPool.length)} connected fit option${Math.min(tradeForFitChoiceCount, tradeForFitPool.length) === 1 ? "" : "s"} available`
                      : tradeForFitEnabled
                        ? "No lower-tier connected fits are available for this player."
                        : "Available after the Year 2 NBA Finals."}
                  </div>
                </button>
              </div>
              <div className="mt-5 rounded-[22px] border border-amber-200/14 bg-amber-300/8 px-4 py-4 text-sm text-slate-100">
                Skipping the replacement board still restores {pendingTrade.outgoingPlayerName ?? "the outgoing player"} and keeps your roster intact.
              </div>
              <BackToRunLadderButton onClick={backToRunLadder} className="mt-5" />
            </div>
          )}

          {run.stage === "evolution-select" && activeNode && (
            <div className="glass-panel rounded-[30px] p-6 shadow-card">
              <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Evolution Floor</div>
              <h2 className="mt-2 font-display text-3xl text-white">{activeNode.title}</h2>
              <p className="mt-3 text-sm leading-7 text-slate-300">
                  ...
                  <div className="relative p-5 lg:p-7">
                    <div>
                      <div>
                        ...
                        <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-300">
                          The result is locked in. Watch the precomputed game play out, then review the final box score and player matchup results.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
                      <div className="rounded-[30px] border border-white/12 bg-black/22 p-5">
                        ...
                        {simulationComplete ? (
                          <div className="mt-6 flex flex-col items-center gap-3 rounded-[24px] border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.035))] px-5 py-5 sm:flex-row sm:justify-center">
                            <button
                              type="button"
                              onClick={run.nodeResult?.passed ? continueAfterFaceoff : abortRun}
                              className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-semibold text-slate-950 shadow-[0_14px_34px_rgba(255,255,255,0.12)] transition hover:scale-[1.02]"
                            >
                            ...
                            {!run.nodeResult?.passed ? (
                              <button
                                type="button"
                                onClick={handleBackToHome}
                                className="inline-flex items-center justify-center gap-2 rounded-full border border-white/12 bg-white/6 px-7 py-3.5 text-sm font-semibold text-white transition hover:bg-white/10"
                              >
                                Back to Home
                              </button>
                            ) : null}
                    ...
                    {simulationComplete ? (
                      <div className="mt-6 rounded-[28px] border border-white/10 bg-white/5 p-5">
                        <div className="text-[10px] uppercase tracking-[0.24em] text-slate-400">Player Matchup Results</div>
                        <div className="mt-4 space-y-4">
                          {run.nodeResult.faceoffResult.matchups.map((matchup) => (
                            <FaceoffMatchupRow
                              ...
                              matchup={matchup}
                              playerStats={simulationResult.playerStats}
                              ownedPlayerIds={runOwnedPlayerIds}
                              trainedPlayerIds={runDisplayTrainedPlayerIds}
                              coachTeamKey={runCoachTeamKey}
                              allStarBonusBadges={runAllStarBonusBadges}
          ...
          {run.stage === "chance-spin" && activeNode?.type === "chance" && (
            <div className="glass-panel overflow-hidden rounded-[30px] border border-yellow-200/16 bg-[radial-gradient(circle_at_24%_24%,rgba(250,204,21,0.16),transparent_32%),linear-gradient(135deg,rgba(8,13,22,0.98),rgba(12,20,31,0.96),rgba(4,9,16,0.98))] p-6 shadow-card">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="text-xs uppercase tracking-[0.24em] text-yellow-100/70">Team Wheel</div>
                  <h2 className="mt-2 font-display text-3xl text-white">Spin for your draft board</h2>
                  <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">
                    Land on one NBA team, then choose 1 of 5 {getChanceWheelTierLabel(activeNode)} players from that team.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={spinChanceWheel}
                  disabled={chanceWheelSpinning}
                  className={clsx(
                    "inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold transition",
                    chanceWheelSpinning
                      ? "border border-white/10 bg-white/10 text-white/52"
                      : "bg-white text-slate-950 hover:bg-yellow-100",
                  )}
                >
                  <RefreshCcw size={16} className={clsx(chanceWheelSpinning && "animate-spin")} />
                  {chanceWheelSpinning ? "Spinning..." : "Spin Team Wheel"}
                </button>
              </div>

              <div className="mt-8 flex justify-center overflow-hidden">
                <div className="relative flex aspect-square w-full max-w-[950px] items-center justify-center p-6">
                  <div className="absolute left-[-6px] top-1/2 z-20 h-0 w-0 -translate-y-1/2 border-b-[17px] border-l-[28px] border-t-[17px] border-b-transparent border-l-white border-t-transparent drop-shadow-[0_8px_18px_rgba(255,255,255,0.2)]" />
                  <div
                    className="relative h-full w-full overflow-hidden rounded-full border-[7px] border-white/80 shadow-[0_24px_70px_rgba(0,0,0,0.38),inset_0_0_0_1px_rgba(255,255,255,0.44)] transition-transform duration-[5600ms] ease-[cubic-bezier(0.08,0.82,0.12,1)]"
                    style={{
                      background: chanceWheelBackground,
                      transform: `rotate(${chanceWheelRotation}deg)`,
                    }}
                  >
                    <div className="pointer-events-none absolute inset-0 rounded-full bg-[radial-gradient(circle_at_center,transparent_0_18%,rgba(255,255,255,0.2)_18.5%,transparent_19%),radial-gradient(circle_at_34%_28%,rgba(255,255,255,0.26),transparent_14%),radial-gradient(circle_at_center,transparent_0_64%,rgba(0,0,0,0.22)_100%)]" />
                    {nbaTeams.map((team, index) => {
                      const sliceDegrees = 360 / nbaTeams.length;
                      const boundaryAngle = sliceDegrees * index;
                      const centerAngle = boundaryAngle + sliceDegrees / 2;
                      const markerAngle = centerAngle - 90;
                      return (
                        <div key={team.name}>
                          <div
                          ...
                          />
                          <div
                            className="absolute left-1/2 top-1/2 flex h-9 w-[190px] -ml-[95px] -mt-[18px] items-center justify-end gap-2 pr-4 text-right text-[clamp(0.56rem,0.9vw,0.78rem)] font-black uppercase leading-none tracking-[0.02em] text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.72)]"
                            style={{
                              transform: `rotate(${markerAngle}deg) translateX(min(30vw, 285px))`,
                              transformOrigin: "50% 50%",
                            }}
                          >
                            <span className="max-w-[118px] truncate">{getChanceWheelTeamLabel(team.name)}</span>
                            {team.logo ? (
                              <span className="flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/75 bg-white shadow-[0_2px_7px_rgba(0,0,0,0.32)]">
                                <img
                                  src={team.logo}
                                  alt=""
                    ...
                    <div className="absolute left-1/2 top-1/2 flex h-[clamp(7rem,16vw,12rem)] w-[clamp(7rem,16vw,12rem)] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-[7px] border-white/85 bg-[linear-gradient(180deg,#ffffff,#dbeafe)] text-center shadow-[0_10px_24px_rgba(0,0,0,0.28),inset_0_1px_0_rgba(255,255,255,0.85)]">
                      <div>
                        <div className="text-[clamp(0.7rem,1.1vw,0.95rem)] uppercase tracking-[0.2em] text-slate-500">NBA</div>
                        <div className="mt-1 text-[clamp(2.1rem,4.2vw,4.8rem)] font-black leading-none text-slate-950">30</div>
                        <div className="mt-1 text-[clamp(0.7rem,1.1vw,0.95rem)] uppercase tracking-[0.16em] text-slate-500">Teams</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-5 space-y-3">
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <div className="mt-2 text-sm leading-6 text-slate-300">
                      Every eligible player for this run is already available from Floor 1. Later reward boards are controlled by floor tier rules and your run settings.
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                    <div className="max-w-3xl">
                      ...
                      <p className="mt-2 max-w-3xl text-[13px] leading-6 text-white/82 lg:text-sm">
                        Year 3 Floor 60 is down. Your championship run is complete.
                      </p>
                        ...
                        <div className="rounded-[22px] border border-amber-100/16 bg-black/14 px-4 py-4">
                          ...
                          <div className="mt-2 text-sm leading-7 text-white/76">
                            {activeNode?.id === "act-one-boss"
                              ...
                              : "Choose 1 reward player and strengthen your roster before the next floor."}
                          </div>
                        </div>
              ...
              {isFailureOverlay && run.nodeResult?.failureRewards ? (
                <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(300px,0.9fr)]">
                  {failureAutopsyCards.length > 0 ? (
                    <div className="overflow-hidden rounded-[30px] border border-white/14 bg-[radial-gradient(circle_at_top_left,rgba(125,211,252,0.16),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.96),rgba(35,24,54,0.82),rgba(8,13,24,0.94))] p-4 shadow-[0_24px_60px_rgba(0,0,0,0.28),inset_0_1px_0_rgba(255,255,255,0.06)] sm:p-5">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <div className="inline-flex items-center gap-2 rounded-full border border-sky-100/18 bg-sky-300/10 px-3 py-1.5 text-[11px] uppercase tracking-[0.24em] text-sky-100/84">
                            <Target size={14} />
                            Run Autopsy
                          </div>
                          <div className="mt-3 text-lg font-semibold text-white">
                            What this run was telling you
                          </div>
                          <div className="mt-1.5 max-w-2xl text-sm leading-6 text-slate-200/82">
                            A quick snapshot of the lineup, weak spots, and the final gap from this attempt.
                          </div>
                        </div>
                      </div>
                      <div className="mt-4 grid gap-3 md:grid-cols-3">
                        {failureAutopsyCards.map((card) => (
                          <div
                            key={card.title}
                            className="rounded-[24px] border border-white/12 bg-[linear-gradient(180deg,rgba(255,255,255,0.09),rgba(255,255,255,0.035))] p-4 shadow-[0_14px_34px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.06)]"
                          >
                            <div className="text-[10px] uppercase tracking-[0.22em] text-sky-100/72">
                              {card.title}
                            </div>
                            <div className="mt-3 space-y-2">
                              {card.rows.map((row) => (
                                <div
                                  ...
                                  className="flex min-h-[46px] items-center justify-between gap-3 rounded-[16px] border border-white/8 bg-black/18 px-3 py-2.5"
                                >
                                  <div className="text-[10px] uppercase tracking-[0.18em] text-slate-300/64">
                                    {row.label}
                                  </div>
                                  <div className="max-w-[62%] text-right text-sm font-semibold leading-6 text-white/92">
                                    {row.value}
                                  </div>
                                </div>
                              ))}
